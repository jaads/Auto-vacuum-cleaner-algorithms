...
def breadth_first_search(matrix):
    ...
    while frontier:
        ...
        if current_pos not in already_checked_list:
            ...
            # Append all children to the FIFO queue
            for edge in ordered_edges:
                if not (matrix[edge] in wall_chars) or (edge in already_checked_list):
                    frontier.put(edge)
                    if frontier.qsize() > frontier_max_len:
                        frontier_max_len = frontier.qsize()
...
if __name__ == '__main__':
    ...
    map_matrix = maze_map_to_matrix(maze_map_map1)
    breadth_first_search(map_matrix)
